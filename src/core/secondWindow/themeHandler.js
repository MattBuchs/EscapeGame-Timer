 ...
 */
async function initThemeHandler() {
	// Récupérer et appliquer le thème sauvegardé depuis le process principal
	let savedTheme = null;
	try {
		savedTheme = await ipcRenderer.invoke("get-current-theme");
	} catch (error) {
		console.warn(
			"Impossible de récupérer le thème courant via IPC:",
			error,
		);
	}
...
export { applyTheme, initThemeHandler };
